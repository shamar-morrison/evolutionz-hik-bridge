...
When both `HIK_DEBUG_AVAILABLE_SLOTS=1` and `HIK_DEBUG_AVAILABLE_SLOTS_CARD_NOS` are set, the bridge also logs focused device evidence, focused bulk page traces, direct `CardNoList` card probes, direct user fuzzy probes, and a final comparison report. The focused device evidence includes `getCapabilities()`, `UserInfo/Count`, and `CardInfo/Count` alongside the bridge's bulk scan totals, which makes it easier to spot cases where the laptop only sees part of the device dataset.
The direct card probe is now status-aware. If the device firmware rejects `CardNoList` with `badParameters`, the report marks that probe as `unsupported` instead of implying the card was cleanly missing from the device.
The focused user fuzzy probes search by both the slot token and the focused card number string. For example, use `HIK_DEBUG_AVAILABLE_SLOTS_PLACEHOLDER_NAMES=P55` and `HIK_DEBUG_AVAILABLE_SLOTS_CARD_NOS=0105451261` to compare bulk pagination against focused probes for both `P55` and `0105451261`.
